...
public class ContractServiceImplement implements IContract {
    ...
    public void add() {
        ...
        ReadAndWrite.WriteBookingQueueToCSV(bookingQueue, bookingQueueFilePath, false);
                ...
                if (contractList.get(i).getIdContract().equals(contractIdEdit)) {
                    ...
                    switch (choice) {
                        case 1:
                            ...
                            double newAmountOfDeposit = Double.parseDouble(sc.nextLine());
                            contractList.get(i).setAmountOfDeposit(newAmountOfDeposit);
                            ReadAndWrite.WriteContractStringToCSV(contractList, contractListPath, false);
                            break;
                        case 2:
                            System.out.println("Enter new total pay: ");
                            double newTotalPay = Double.parseDouble(sc.nextLine());
                            contractList.get(i).setTotalPay(newTotalPay);
                            ReadAndWrite.WriteContractStringToCSV(contractList, contractListPath, false);
                            break;
                        case 3:
                            System.out.println("Enter new customer's ID: ");
                            String newCustomerID = sc.nextLine();
                            contractList.get(i).setIdCustomer(newCustomerID);
                            ReadAndWrite.WriteContractStringToCSV(contractList, contractListPath, false);
                            break;
                        case 0:
                            break;
                    ...
                    System.out.println("Cannot find the contract's code for editing! \n Please enter again ");
                }
            }
        } catch (NumberFormatException e) {
            e.printStackTrace();
